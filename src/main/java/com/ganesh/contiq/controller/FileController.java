...
public class FileController {
    ...
    @PostMapping("/files")
    public ResponseEntity<SuccessResponseDTO> uploadFiles(@RequestParam("files") List<MultipartFile> files, @AuthenticationPrincipal Jwt jwt) throws IOException {
        ...
        return new ResponseEntity<>(new SuccessResponseDTO("Files uploaded successfully"), HttpStatus.CREATED);
    }

    @GetMapping("/files")
    public ResponseEntity<FileMetaDataListDTO> getFilesByUserId( @AuthenticationPrincipal Jwt jwt) {
    ...
    public ResponseEntity<FileContentDTO> getFileContentById(@PathVariable("file-id") String fileId, @AuthenticationPrincipal Jwt jwt) throws FileAccessDeniedException, FileNotFoundException {
        String userId= JwtUtil.getUserId(jwt);
        return new ResponseEntity<>(fileService.getFileContentById(fileId,userId), HttpStatus.OK);
    }

    @GetMapping("/files/search")
    public ResponseEntity<FileMetaDataListDTO> getFilesByUserIdAndKeyword(@RequestParam("keyword") String keyword, @AuthenticationPrincipal Jwt jwt) {
